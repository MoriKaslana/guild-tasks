...
const AppSidebar = () => {
  const { currentUser, logout, switchRole, chatMessages } = useGame();
  ...
  const [lastSeenCount, setLastSeenCount] = useState(() => {
    return parseInt(localStorage.getItem("tavern_last_seen") || "0", 10);
  });

  // Mark as read when user is on tavern page
  useEffect(() => {
    if (location.pathname === "/tavern") {
      setHasUnread(false);
      setLastSeenCount(chatMessages.length);
      localStorage.setItem("tavern_last_seen", String(chatMessages.length));
    }
  }, [location.pathname, chatMessages.length]);

  // Show badge when new messages arrive and user is NOT on tavern
  useEffect(() => {
    if (
      location.pathname !== "/tavern" &&
      chatMessages.length > lastSeenCount
    ) {
      setHasUnread(true);
    }
  }, [chatMessages.length]);
  ...
  const handleSwitchRole = () => {
    ...
    const targetRole =
      currentUser.role === "guild_master" ? "adventurer" : "guild_master";
  ...
  return (
    ...
    >
              ...
              {navItems.map((item) => (
                <SidebarMenuItem key={item.url}>
                  <SidebarMenuButton asChild tooltip={item.title}>
                    ...
                    >
                      <span className="text-xl shrink-0 flex items-center justify-center w-6 relative">
                        {item.icon}
                        {(item as any).hasNotif && (
                          <span className="absolute -top-1 -right-1 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-red-600 text-[8px] text-white font-bold animate-bounce shadow-md">
                            !
                          </span>
                        )}
                      </span>
                      ...
                      <span
                        className={`font-body transition-all duration-300 ease-in-out overflow-hidden whitespace-nowrap flex items-center gap-1.5 ${
                          collapsed ? "w-0 opacity-0" : "w-auto opacity-100"
                        }`}
                      >
                        {item.title}
                        {(item as any).hasNotif && !collapsed && (
                          <span className="flex h-4 w-4 items-center justify-center rounded-full bg-red-600 text-[8px] text-white font-bold animate-pulse">
                            !
                          </span>
                        )}
                      </span>
      ...
      <SidebarFooter className="border-t border-border p-3 space-y-3 transition-all duration-300">
        {currentUser && (
          <div
            className={`flex items-center gap-2 p-1 rounded-lg bg-secondary/20 ${collapsed ? "justify-center" : "justify-between"}`}
          >
            <div
            ...
            <div
              className={`${collapsed ? "scale-90 transition-transform" : ""}`}
            >
              <InvitationInbox />
            </div>
          </div>
        ...
        {canSwitch && (
          ...
          >
            <RefreshCcw
              className={`h-4 w-4 shrink-0 ${collapsed ? "" : "mr-2"}`}
            />
            {!collapsed && (
        ...
        >
          ...
          {!collapsed && (
            <span className="ml-2 font-body animate-in fade-in">Keluar</span>
          )}
        </Button>
      </SidebarFooter>
    </Sidebar>
