...
export const userService = {
  ...
  async fetchAllUsers(): Promise<User[]> {
    ...
    return data.map((u: any) => ({
      ...
      lastQuestCompletedAt: u.last_quest_completed_at
        ? Number(u.last_quest_completed_at)
        : null,
      consecutiveLateCount:
        u.consecutive_late_count || u.consecutivelatecount || 0,
      debuffImmunity: u.debuff_immunity || u.debuffimmunity || false,
      stagnantSoulCounter:
        u.stagnant_soul_counter || u.stagnantsoulcounter || 0,
  ...
  async updateUserDb(user: User): Promise<void> {
    const { error } = await supabase
      ...
      .update({
        ...
        debuff_immunity: user.debuffImmunity,
