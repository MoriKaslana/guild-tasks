...
export const GameProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const fetchQuests = useCallback(async () => {
    ...
    try {
      const mappedQuests = await questService.fetchQuestsDb(
        currentUser.guildId,
      );
    ...
    if (users.length > 0 && achievements.length > 0) {
      setAchievements((prev) => {
        ...
        const next = prev.map((ach) => {
          const unlocked = users
            .filter((u) => u.achievements?.includes(ach.id))
            .map((u) => u.id);
          if (unlocked.length !== (ach.unlockedBy?.length || 0))
            isChanged = true;
  ...
  const applyBuffsDebuffs = useCallback(
    (
      user: User,
      quest: Quest,
      submittedAt: number,
      ...
      if (timeLeft >= 24 * 60 * 60 * 1000) {
        u = gamificationService.addBuff(
          u,
          "Adventurer's Haste",
          BUFF_DURATION,
          quest.id,
        );
        toast("⚡ Buff: Adventurer's Haste", {
          description: "+50% XP! Submitted 24h+ early.",
        });
      }
      if (quest.difficulty === "hard") {
        ...
        toast("📚 Buff: Scholar's Focus", {
          description: "+20% XP! Completed a Hard quest.",
        });
      }
      ...
      if (day === 0 || day === 6) {
        ...
        toast("🗡️ Buff: Weekend Warrior", {
          description: "+10% XP! Weekend dedication.",
        });
      }
      ...
      if (timeLeft > 0 && timeLeft < totalTime * 0.1) {
        ...
        toast("🎯 Buff: Clutch Player", {
          description: "Submitted just in time!",
        });
      }
      const timeSinceAccept =
        submittedAt - (quest.acceptedAt || quest.createdAt);
      if (timeSinceAccept < 60 * 60 * 1000) {
        ...
        toast("⚔️ Buff: First Strike", {
          description: "Completed within an hour!",
        });
      }
      const recentSubmissions = allQuests.filter(
        (q) =>
          q.assignedTo === u.id &&
          q.submittedAt &&
          submittedAt - q.submittedAt < CHAIN_WINDOW &&
          q.id !== quest.id,
      );
      if (recentSubmissions.length >= 2) {
        ...
        toast("🔗 Buff: Chain Quest (Combo)!", {
          description: "+15% XP! 3 quests in 24 hours.",
        });
      }
      if (quest.difficulty === "legendary") {
        ...
        toast("✨ Ultimate Buff: Aura of Purity!", {
          description: "All debuffs cleared! Immunity to next debuff granted.",
        });
        u = gamificationService.addBuff(
          u,
          "Scholar's Focus",
      ...
      if (submittedAt > quest.deadline) {
        u = gamificationService.addDebuff(
          u,
        ...
        if (u.debuffs.includes("Cursed Procrastination"))
          toast("🐌 Debuff: Cursed Procrastination", {
            description: "-10 XP penalty for late submission.",
          });
      ...
      const activeQuests = allQuests.filter(
        (q) => q.assignedTo === u.id && q.status === "accepted",
      );
      if (activeQuests.length > 5) {
        ...
        if (u.debuffs.includes("Slacker's Fatigue"))
          toast("😴 Debuff: Slacker's Fatigue", {
            description: "-5% XP on next quest.",
          });
      }
      return u;
    },
  ...
  const checkInactivityDebuff = useCallback(
    (user: User, allQuests: Quest[]): User => {
      let u = gamificationService.cleanExpiredEffects(user);
  ...
  const register = async (
    ...
    try {
      ...
      if (existingUser) {
        if (role === "guild_master" && existingUser.is_guild_master) {
          toast.error("Role Guild Master sudah ada!");
          return false;
        }
        if (role === "adventurer" && existingUser.is_adventurer) {
          toast.error("Role Adventurer sudah ada!");
          return false;
        }
        const newGuildId =
        ...
        await authService.updateUser(existingUser.id, {
          ...
          guild_id: newGuildId,
          is_guild_master:
            role === "guild_master" ? true : existingUser.is_guild_master,
          is_adventurer:
            role === "adventurer" ? true : existingUser.is_adventurer,
        });
        await fetchUsers();
      ...
      await authService.insertUser({
        ...
        level: newUser.level,
        buffs: newUser.buffs,
        debuffs: newUser.debuffs,
        active_buffs: newUser.activeBuffs,
        active_debuffs: newUser.activeDebuffs,
        guild_id: assignedGuildId,
        quests_completed: newUser.questsCompleted,
        joined_at: newUser.joinedAt,
        last_quest_completed_at: newUser.lastQuestCompletedAt,
        consecutive_late_count: newUser.consecutiveLateCount,
        debuff_immunity: newUser.debuffImmunity,
        stagnant_soul_counter: newUser.stagnantSoulCounter,
        rusty_equipment: newUser.rustyEquipment,
        broken_shield_quests: newUser.brokenShieldQuests,
        is_guild_master: newUser.isGuildMaster,
  ...
  const login = async (
    ...
    try {
      ...
      const loggedInUser: User = {
        ...
        joinedAt: data.joined_at || Date.now(),
        lastQuestCompletedAt: data.last_quest_completed_at,
        consecutiveLateCount: data.consecutive_late_count || 0,
        debuffImmunity: data.debuff_immunity || false,
        stagnantSoulCounter: data.stagnant_soul_counter || 0,
        rustyEquipment: data.rusty_equipment || false,
        brokenShieldQuests: data.broken_shield_quests || [],
  ...
  const switchRole = async (newRole: Role): Promise<void> => {
    ...
    const hasAccess =
      newRole === "guild_master"
        ...
        : currentUser.isAdventurer;
    if (!hasAccess) {
      toast.error("Role tidak diizinkan.");
      return;
    }
  ...
  const submitQuest = async (
    ...
    if (file) {
      try {
        submissionUrl = await new Promise<string>((resolve, reject) => {
        ...
        toast.warning("Gagal membaca file bukti. Quest tetap dikumpulkan.");
      }
    }

    try {
      await questService.submitQuest(
        questId,
      ...
      const updatedUser = applyBuffsDebuffs(
        currentUser,
        tempQuest,
        now,
        quests,
  ...
  const approveQuest = async (questId: string): Promise<void> => {
    ...
    try {
      await questActionService.approveQuestDb(
        questId,
        new Date(now).toISOString(),
      );
      let xpBreakdown = gamificationService.calcXpWithModifiers(
        adventurer,
        quest,
      );
      ...
      if (
        ...
        const loserQuest = quests.find(
          (q) =>
            q.assignedTo === quest.duelOpponentId &&
            q.isDuel &&
            q.duelStatus === "accepted",
        );
        if (loserUser && loserQuest) {
          ...
          await updateUserInDb({
            ...loserUser,
            xp: loserNewXp,
            level: gamificationService.calcLevel(loserNewXp),
          });
      ...
      const currentQuestWithCompleteTime = {
        ...quest,
        status: "completed" as const,
        completedAt: now,
      };
      const newAwards = achievementService.checkAchievements(
        updated,
        quests,
        currentQuestWithCompleteTime,
      );

      if (newAwards.length > 0) {
        // Gabungin achievement lama + baru tanpa duplikat
        updated.achievements = Array.from(
          new Set([...(adventurer.achievements || []), ...newAwards]),
        );
  ...
  const rejectQuest = async (questId: string): Promise<void> => {
    ...
    try {
      ...
      const adventurer = users.find((u) => u.id === quest.assignedTo);
      if (adventurer) {
        let updated = gamificationService.addDebuff(
          adventurer,
          "Broken Shield",
          48 * 60 * 60 * 1000,
          questId,
        ...
        updated = {
          ...updated,
          brokenShieldQuests: [...updated.brokenShieldQuests, questId],
        };
        await updateUserInDb(updated);
  ...
  const sendDuelChallenge = async (myQuestId: string, targetUserId: string) => {
    if (!currentUser) return;
    const myQuest = quests.find(
      (q) => q.id === myQuestId && q.assignedTo === currentUser.id,
    );
    ...
    if (!myQuest || !targetQuest) {
      toast.error("Syarat duel tidak terpenuhi.");
      return;
    }
    try {
      await duelService.sendChallenge(
        myQuest.id,
        targetQuest.id,
        currentUser.id,
        targetUserId,
  ...
  const respondToDuel = async (
    ...
    const challengerQuest = quests.find(
      (q) =>
    ...
    try {
      ...
      if (action === "accept") {
        sendMessage(
          `⚔️🔥 [DUEL DIMULAI] @${currentUser.username} menerima tantangan!`,
        );
        toast.success("Duel dimulai!");
      }
      await fetchQuests();
    } catch (err) {
      toast.error("Gagal respon duel.");
    }
  };
      ...
      if (newAwards.length > 0) {
        ...
        const updatedUser = {
          ...
          achievements: Array.from(
            new Set([...(currentUser.achievements || []), ...newAwards]),
          ),
        };
        ...
        newAwards.forEach((id) => {
          if (id === "Talkative") {
            ...
            toast.success("🏆 ACHIEVEMENT UNLOCKED: Tavern Regular!", {
              description:
                "10 pesan terkirim. Kamu sudah jadi pelanggan tetap!",
            });
          } else {
            toast.success(`🏆 Achievement Unlocked: ${id}`);
          }
  ...
  const kickMember = async (memberId: string) => {
    ...
    try {
      ...
      // 2. Reset semua quest yang masih dipegang (accepted/submitted) kembali ke open
      await supabase
        .from("quests")
        .update({
          status: "open",
      ...
      setUsers((prev) =>
        prev.map((u) => (u.id === memberId ? { ...u, guildId: "" } : u)),
      );
      toast.success("Member dikeluarkan dari Guild!");
      await Promise.all([fetchUsers(), fetchQuests()]);
    } catch (err) {
      toast.error("Gagal kick.");
    }
  };

  const changeAvatar = (avatar: string) => {
    if (currentUser) updateUserInDb({ ...currentUser, avatar });
  };
  ...
  const cleanOldChats = async () => {
    try {
      ...
      console.error("Auto-clean failed");
    }
  };
