...
const QuestBoard = () => {
  ...
  useEffect(() => {
    if (isGM && currentUser) {
      ...
      const hasSeen = localStorage.getItem(
        `gm_onboarding_done_${currentUser.id}`,
      );
      if (!hasSeen) {
  ...
  const myActive = quests.filter(
    (q) => q.assignedTo === currentUser?.id && q.status === "accepted",
  );
  const mySubmitted = quests.filter(
    (q) => q.assignedTo === currentUser?.id && q.status === "submitted",
  );
  const myCompleted = quests.filter(
    (q) => q.assignedTo === currentUser?.id && q.status === "completed",
  );

  // Hanya tampilkan popup ke yang DITANTANG, bukan penantangnya
  const incomingDuel = quests.find(
    (q) =>
      q.isDuel &&
      q.duelStatus === "pending" &&
      q.duelOpponentId === currentUser?.id &&
      q.challengerId !== currentUser?.id,
  );

  const challenger = users.find((u) => u.id === incomingDuel?.challengerId);

  // Notifikasi untuk PENANTANG saat duel diterima atau ditolak
  const prevOutgoingDuelRef = useRef<{
    id: string;
    status: string | null;
  } | null>(null);

  useEffect(() => {
    const myOutgoingDuel = quests.find(
      (q) =>
        q.isDuel &&
        q.challengerId === currentUser?.id &&
        q.assignedTo === currentUser?.id,
    );

    const prev = prevOutgoingDuelRef.current;

    if (myOutgoingDuel) {
      const currentStatus = myOutgoingDuel.duelStatus || null;
      if (prev?.status === "pending" && currentStatus === "accepted") {
        const opponentName =
          users.find((u) => u.id === myOutgoingDuel.duelOpponentId)?.username ||
          "Lawanmu";
        toast.success(
          `⚔️ @${opponentName} menerima tantangan duelmu! Mulailah bertarung!`,
        );
      }
      prevOutgoingDuelRef.current = {
        id: myOutgoingDuel.id,
        status: currentStatus,
      };
    } else {
      if (prev?.status === "pending") {
        toast.info(
          "😤 Tantangan duelmu telah ditolak. Coba tantang yang lain!",
        );
      }
      prevOutgoingDuelRef.current = null;
    }
  }, [quests]);
            ...
            >
              ...
              <div className="relative z-10 text-center space-y-6">
                ...
                <div className="flex gap-3 pt-4 font-heading">
                  <Button
                    onClick={() =>
                      respondToDuel && respondToDuel(incomingDuel.id, "accept")
                    }
                  ...
                  <Button
                    onClick={() =>
                      respondToDuel && respondToDuel(incomingDuel.id, "reject")
                    }
                    variant="outline"
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent className="bg-card border-border max-w-lg">
                ...
                <form onSubmit={handleCreate} className="space-y-4 pt-4">
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button
                            variant="outline"
                            className={cn(
                              "flex-1 min-w-[150px] justify-start text-left font-body bg-secondary border-border",
                  ...
                  <Button
                    type="submit"
                    className="w-full font-heading h-12 text-lg"
                  >
                    Buat Tugas
                  </Button>
                </form>
...
const Section = ({ title, count, children, variant, icon }: SectionProps) => {
  const styles = {
    ...
    active:
      "border-blue-500/30 bg-blue-500/5 shadow-[inset_0_0_30px_rgba(59,130,246,0.03)]",
    review:
      "border-gold/30 bg-gold/5 shadow-[inset_0_0_30px_rgba(212,175,55,0.03)]",
    completed:
      "border-emerald-500/30 bg-emerald-500/5 shadow-[inset_0_0_30px_rgba(16,185,129,0.03)]",
  };
    ...
    >
      <div className="absolute -top-4 left-6 flex items-center gap-2.5 px-4 py-1.5 rounded-lg bg-background border border-inherit shadow-lg z-10">
        ...
        <h2 className="font-heading text-sm tracking-widest uppercase text-foreground whitespace-nowrap">
          {title}{" "}
          <span className="ml-1 opacity-50 font-body text-xs lowercase">
            ({count})
          </span>
        </h2>
        <span
          className={cn("w-1.5 h-1.5 rounded-full ml-1", dotColors[variant])}
        />
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 relative">
        <AnimatePresence mode="popLayout">
          {count === 0 ? (
            ...
            >
              <div className="text-5xl mb-3 opacity-10 font-heading select-none italic text-gold">
                Kosong
              </div>
              <p className="text-muted-foreground font-body text-sm italic tracking-wide">
                Tidak ada catatan tugas ditemukan.
              </p>
            </motion.div>
          ) : (
            children
