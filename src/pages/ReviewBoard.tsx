              ...
              >
                <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-3 mt-3 text-xs text-muted-foreground">
                      <span>
                        ...
                        <strong className="text-gold">
                          @{adventurer?.username || "Unknown"}
                        </strong>
                      </span>
                      <span className="flex items-center gap-1">
                        <Zap className="h-3 w-3 text-gold" /> {q.xpReward} XP
                      </span>
                      {q.wasRejected && (
                  ...
                  <div className="flex gap-2 shrink-0">
                    ...
                    <Button
                      onClick={() =>
                        handleActionClick(q.id, q.title, "approve")
                      }
                ...
                {q.submissionUrl && (
                  <div className="mt-4 pt-4 border-t border-white/5">
                    <p className="text-[10px] font-heading text-muted-foreground uppercase tracking-widest mb-2 flex items-center gap-1">
                      <Paperclip className="h-3 w-3" /> Bukti Pengerjaan
                    </p>
                    {q.submissionUrl.startsWith("data:image/") ||
                    q.submissionUrl.match(/\.(jpg|jpeg|png|webp|gif)$/i) ? (
                      <a
                        href={q.submissionUrl}
                        target="_blank"
                      ...
                      >
                        <img
                          ...
                          className="max-h-48 rounded-md object-contain border border-white/10 hover:border-gold/40 transition-colors"
                        />
                      </a>
                    ) : (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 text-xs font-heading text-gold hover:text-gold/80 bg-gold/5 border border-gold/20 px-3 py-2 rounded-md transition-colors"
                      >
        ...
        <DialogContent className="sm:max-w-md border-gold/30 bg-background/95 backdrop-blur-md">
          <DialogHeader>
            <DialogTitle className="font-heading text-lg flex items-center gap-2">
              <AlertTriangle
                className={`h-5 w-5 ${confirmModal.type === "approve" ? "text-gold" : "text-crimson"}`}
              />
              {confirmModal.type === "approve"
                ? "Konfirmasi Penerimaan Tugas"
                : "Konfirmasi Penolakan Tugas"}
            </DialogTitle>
            <DialogDescription className="font-body text-sm pt-2">
              {confirmModal.type === "approve" ? (
                <>
                  Apakah Anda yakin ingin{" "}
                  <strong className="text-gold">MENERIMA</strong> laporan tugas{" "}
                  <span className="italic text-foreground">
                    "{confirmModal.questTitle}"
                  </span>
                  ? Tindakan ini akan mencairkan XP ke Adventurer terkait.
                </>
              ) : (
                <>
                  Apakah Anda yakin ingin{" "}
                  <strong className="text-crimson">MENOLAK</strong> laporan
                  tugas{" "}
                  <span className="italic text-foreground">
                    "{confirmModal.questTitle}"
                  </span>
                  ? Tugas akan dikembalikan ke papan tugas Adventurer.
                </>
              )}
            </DialogDescription>
          </DialogHeader>
